...
def n_cen_LRG(M_h, logM_cut, sigma):
    ...
    """
    return 0.5 * math.erfc((logM_cut - np.log10(M_h)) / (1.41421356 * sigma)) * 0 # TODO: UNDO
...
def N_sat_elg(M_h, M_cut, kappa, M_1, alpha, A_s=1.0, alpha1=0.0, beta=0.0):
    ...
    ) * 0 # TODO: UNDO # + beta*(M_h/M_1)**(-alpha1)/100
...
def N_cen_ELG_v1(M_h, p_max, Q, logM_cut, sigma, gamma, Anorm=1):
    ...
    ) * 0 # TODO: UNDO # + 0.5/Q*(1 + math.erf((logM_h-logM_cut-0.8)*3))
...
def N_cen_QSO(M_h, logM_cut, sigma):
    ...
    """
    mass_bin_edges = 10**10 * np.logspace(0,6,31)
    mass_lower = mass_bin_edges[5]
    mass_upper = mass_bin_edges[6]
    if M_h - mass_lower < 0:
        return 0
    # elif M_h - mass_upper > 0:
    #     return 0
