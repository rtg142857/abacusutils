...
def compute_fast_NFW(
    ...
    x_h = Lbox/100 * np.arange(100).repeat(1e4).reshape((-1, 1e4)).flatten("F") # np.tile(np.array(range(100)), 10000) # [0, 1, 2, ..., 0, 1, 2, ..., 0, 1, 2...]
    y_h = Lbox/100 * np.arange(100).repeat(100).repeat(100).reshape((-1, 100)).T.flatten() # np.tile(np.repeat(np.array(range(100)), 100), 100)
    z_h = Lbox/100 * np.repeat(np.arange(100), 1e4)
