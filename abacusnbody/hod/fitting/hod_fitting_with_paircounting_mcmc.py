...
def fit_HOD(path_config_filename, save_chains=False):
    ...
    sample_result = sample_chain(target_wp_dict=target_wp,
    ...
    best_fit = sample_result["x"]
    print("Best params:")#, best_fit, flush=True)
    print_hod_values(best_fit)
    print("Chi squared:", sample_result["fun"], flush=True)
    print("Iterations:", sample_result["nit"], flush=True)
    # print("Successful:", OptimizeResult["success"], flush=True)
    print("Final chi^2 calculations:")
    chi2 = log_probability(best_fit, paircounts, tracer_list, target_wp, target_jackknife_inverse, target_ngal, other_stuff_dict_here, clustering_params, minimise=True, wp_limit=(i0, i1), verbose=True)
    # print("Output message:", sample_result["message"], flush=True)

    print("Saving output...", flush=True)
    save_path = fitting_params["sampler_save_path"]
    np.save(save_path+"stoch_sampler_xall.npy", sample_result["xall"])
    np.save(save_path+"stoch_funall.npy", sample_result["funall"])
