...
class FlamingoHOD:
    ...
    def __init__(
        ...
        TODO: Update this documentation because it's broadly incorrect.

        Parameters of the input yaml file:
        ----------
        sim_params: dict
        ...
        self.halo_path = config["Paths"]['halo_path']
        self.params_path = config["Paths"]["params_path"]
        self.ics_path = config["Paths"]["ics_path"]
        self.subsample_dir = config["sim_params"]['subsample_dir']
        self.z_mock = config["Params"]['redshift']
        self.output_dir = config["sim_params"].get('output_dir', './')
    ...
    def staging(self):
        ...
        meta_subsample_dir = Path(self.subsample_dir)
        output_dir = Path(self.output_dir)
        full_subsample_dir = meta_subsample_dir / self.sim_label
        mock_dir = output_dir / self.sim_label
        ...
        params['h'] = sim_params["Cosmology"]["h"]
        ...
        #subsample_files = os.listdir(subsample_dir)
        ...
        for i in range(num_subsample_files):
            ...
            if self.want_ranks:
                raise Exception("Ranks not implemented yet")
        ...
        for i in range(num_subsample_files):
            ...
            self.logger.info(f'Loading simulation file {i}')
            # if (
            #     ('ELG' not in self.tracers.keys())
            #     and ('QSO' not in self.tracers.keys())
            #     and (not self.force_mt)
            ...
            halo_pos = maskedhalos['pos']  # halo positions, Mpc / h
            ...
            hid[halo_ticker : halo_ticker + Nhalos[i]] = halo_ids
            hmultis[halo_ticker : halo_ticker + Nhalos[i]] = halo_multi
            hrandoms[halo_ticker : halo_ticker + Nhalos[i]] = halo_randoms
            hveldev[halo_ticker : halo_ticker + Nhalos[i]] = halo_vel_dev
            hsigma3d[halo_ticker : halo_ticker + Nhalos[i]] = halo_sigma3d
    ...
    def run_hod(
        ...
        ``tracers``: dict
            ...
            If tabulation_mock = True, only use LRGs and ELGs.
        ...
        if tabulation_mock:
            ...
            tabulation_tracers["ELG"] = tracers["ELG"]
